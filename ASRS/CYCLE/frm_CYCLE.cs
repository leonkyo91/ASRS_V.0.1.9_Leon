...
namespace ASRS
{
    public partial class frm_CYCLE : Form
    {
        #region Grid Prarm
        int iCol_Loc = 0;           //儲位
        ...
        private void GridSetRange(ref DataGridView oGrid)
        {
            ...
            oGrid.Columns[iCol_SubLoc].Width = 80; oGrid.Columns[iCol_SubLoc].Name = "對映儲位"; //oGrid.Columns[iCol_SubLoc].Visible = false;
            ...
            oGrid.Columns[iCol_GIB_CUSTOMER].Width = 140; oGrid.Columns[iCol_GIB_CUSTOMER].Name = "Customer Group";   //來貨廠商";
            ...
            oGrid.Columns[iCol_REL_DATE].Width = 100; oGrid.Columns[iCol_REL_DATE].Name = "晶圓入廠日期"; //晶圓入廠日期
        ...
        private void SubQuery()
        {
            ...
            txtLoc1.Text = txtLoc1.Text.Trim().ToUpper();
            txtLoc2.Text = txtLoc2.Text.Trim().ToUpper();
            txtLocID1.Text = txtLocID1.Text.Trim().ToUpper();
            txtLocID2.Text = txtLocID2.Text.Trim().ToUpper();

            txtLocMapping.Text = txtLocMapping.Text.Trim().ToUpper();
            Grid1.RowCount = 0;
            ...
            if (txtLocMapping.Text != "")
            {
                string sMapLoc1 = "";
                sSql = "SELECT LOC1 FROM LOC_MST WHERE LOC = '" + txtLocMapping.Text + "' ";
                if (clsDB.FunRsSql(sSql, ref dbRS))
                {
                    while (dbRS.Read())
                    {
                        sMapLoc1 = dbRS["LOC1"].ToString();
                    }
                    dbRS.Close();
                }


                sSql = "SELECT M.LOC,M.LOCSTS,M.LOCID,M.AVAIL,M.MIXQTY,M.LOC1,D.* ";
                sSql = sSql + "FROM LOC_MST M LEFT JOIN LOC_DTL D ON M.LOC=D.LOC ";
                sSql = sSql + "WHERE M.LOCSTS IN ('S','E') ";
                sSql = sSql + "AND M.LOC IN ('" + txtLocMapping.Text + "','" + sMapLoc1 + "') ";
            ...
            else
            {
                ...
                if (txtLoc1.Text != "")
                {
                    if (txtLoc2.Text != "")
                    {
                        sSql = sSql + "AND M.LOC >= '" + txtLoc1.Text + "' AND M.LOC <= '" + txtLoc2.Text + "' ";
                    }
                    else
                    {
                        sSql = sSql + "AND M.LOC = '" + txtLoc1.Text + "' ";
                    }
                }
                else
                {
                    if (txtLoc2.Text != "")
                    {
                        sSql = sSql + "AND M.LOC = '" + txtLoc2.Text + "' ";
                    }
                }
                ...
                if (txtLocID1.Text != "")
                {
                    if (txtLocID2.Text != "")
                    {
                        sSql = sSql + "AND M.LOCID >= '" + txtLocID1.Text + "' AND M.LOCID <= '" + txtLocID2.Text + "' ";
                    }
                    else
                    {
                        sSql = sSql + "AND M.LOCID = '" + txtLocID1.Text + "' ";
                    }
                }
                else
                {
                    if (txtLocID2.Text != "")
                    {
                        sSql = sSql + "AND M.LOCID = '" + txtLocID2.Text + "' ";
                    }
                }

                if (txtItemNo1.Text != "")
                {
                    sSql = sSql + "AND D.ITEMNO = '" + txtItemNo1.Text + "' ";
                }
                if (txtCustomer.Text != "")
                {
                    sSql = sSql + "AND D.CUSTOMER = '" + txtCustomer.Text + "' ";
                }
            ...
            if (clsDB.FunRsSql(sSql, ref dbRS))
            {
                while (dbRS.Read())
                {
                    ...
                    Grid1[iCol_SubLoc, Grid1.RowCount - 1].Value = dbRS["LOC1"].ToString();
        ...
        {
            string sSql = ""; DbDataReader dbRS = null;
            string sSql_Mapping = string.Empty;
            txtLoc1.Text = txtLoc1.Text.Trim().ToUpper();
            txtLoc2.Text = txtLoc2.Text.Trim().ToUpper();
            txtLocID1.Text = txtLocID1.Text.Trim().ToUpper();
            txtLocID2.Text = txtLocID2.Text.Trim().ToUpper();
            ...
            txtLocMapping.Text = txtLocMapping.Text.Trim().ToUpper();
            txtLocIDMapping.Text = txtLocIDMapping.Text.Trim().ToUpper();
            ...
            if ((txtLocMapping.Text != "") & (txtLocIDMapping.Text != ""))
            {
                //LocMapping
                string sMapLoc1 = ""; string sMapLoc2 = "";
                sMapLoc1 = txtLocMapping.Text;
                sSql = "SELECT LOC1 FROM LOC_MST WHERE LOC = '" + sMapLoc1 + "' ";
                if (clsDB.FunRsSql(sSql, ref dbRS))
                {
                    while (dbRS.Read())
                    {
                        sMapLoc2 = dbRS["LOC1"].ToString();
                    }
                    dbRS.Close();
                }

                //LocIDMapping
                string sMapLoc3 = ""; string sMapLoc4 = "";
                sSql = "SELECT LOC FROM LOC_MST WHERE LOCID = '" + txtLocIDMapping.Text + "' ";
                if (clsDB.FunRsSql(sSql, ref dbRS))
                {
                    while (dbRS.Read())
                    {
                        sMapLoc3 = dbRS["LOC"].ToString();
                    }
                    dbRS.Close();
                }

                sSql = "SELECT LOC1 FROM LOC_MST WHERE LOC = '" + sMapLoc3 + "' ";
                if (clsDB.FunRsSql(sSql, ref dbRS))
                {
                    while (dbRS.Read())
                    {
                        sMapLoc4 = dbRS["LOC1"].ToString();
                    }
                    dbRS.Close();
                }
                ...
                sSql = sSql + "FROM LOC_MST M LEFT JOIN LOC_DTL D ON M.LOC=D.LOC ";
                sSql = sSql + "WHERE M.LOCSTS IN ('S','E') ";
                sSql = sSql + "AND M.LOC IN ('" + sMapLoc1 + "','" + sMapLoc2 + "','" + sMapLoc3 + "','" + sMapLoc4 + "') ";
                sSql = sSql + " ORDER BY M.LOC ";

            }
            else if (txtLocMapping.Text != "")
            {
                string sMapLoc1 = "";
                sSql = "SELECT LOC1 FROM LOC_MST WHERE LOC = '" + txtLocMapping.Text + "' ";
                if (clsDB.FunRsSql(sSql, ref dbRS))
            ...
            else if (txtLocIDMapping.Text != "")
            {
                ...
                if (clsDB.FunRsSql(sSql, ref dbRS))
                {
                    while (dbRS.Read())
                    {
                        sMapLoc = dbRS["LOC"].ToString();
                    }
                    dbRS.Close();
                }

                string sMapLoc1 = "";
                sSql = "SELECT LOC1 FROM LOC_MST WHERE LOC = '" + sMapLoc + "' ";
                if (clsDB.FunRsSql(sSql, ref dbRS))
                {
                    while (dbRS.Read())
                    {
                ...
                sSql = sSql + "FROM LOC_MST M LEFT JOIN LOC_DTL D ON M.LOC=D.LOC ";
                sSql = sSql + "WHERE M.LOCSTS IN ('S','E') ";
                sSql = sSql + "AND M.LOC IN ('" + sMapLoc + "','" + sMapLoc1 + "') ";
                sSql = sSql + " ORDER BY M.LOC ";
            }
            else
            {
                ...
                if (txtLoc1.Text != "")
                {
                    if (txtLoc2.Text != "")
                    {
                        string sLocTmp = string.Empty;
                        sSql_Mapping = "select loc, loc1 from loc_mst where loc >= '" + txtLoc1.Text + "' and  LOC <='" + txtLoc2.Text + "'";
                        if (clsDB.FunRsSql(sSql_Mapping, ref dbRS))
                        {
                            while (dbRS.Read())
                            {
                                sLocTmp += ",'" + dbRS["LOC"].ToString() + "'";
                                sLocTmp += ",'" + dbRS["LOC1"].ToString() + "'";
                            }
                            dbRS.Close();
                            sLocTmp = sLocTmp.Substring(1);
                        }
                        sSql = sSql + "AND M.LOC in (" + sLocTmp + ") ";
                    }
                    else
                    {
                        string sLocTmp = string.Empty;
                        sSql_Mapping = "select loc, loc1 from loc_mst where loc = '" + txtLoc1.Text + "' ";
                        if (clsDB.FunRsSql(sSql_Mapping, ref dbRS))
                        {
                            while (dbRS.Read())
                            {
                ...
                else
                {
                    ...
                    {
                        string sLocTmp = string.Empty;
                        sSql_Mapping = "select loc, loc1 from loc_mst where loc = '" + txtLoc2.Text + "' ";
                        if (clsDB.FunRsSql(sSql_Mapping, ref dbRS))
                        {
                            while (dbRS.Read())
                            {
                ...
                if (txtLocID1.Text != "")
                {
                    ...
                    {
                        string sLocTmp = string.Empty;
                        sSql_Mapping = "select loc, loc1 from loc_mst where LOCID >= '" + txtLocID1.Text + "' and LOCID <= '" + txtLocID2.Text + "' ";
                        if (clsDB.FunRsSql(sSql_Mapping, ref dbRS))
                        {
                            while (dbRS.Read())
                            {
                    ...
                    {
                        string sLocTmp = string.Empty;
                        sSql_Mapping = "select loc, loc1 from loc_mst where LOCID = '" + txtLocID1.Text + "' ";
                        if (clsDB.FunRsSql(sSql_Mapping, ref dbRS))
                        {
                            while (dbRS.Read())
                            {
                ...
                else
                {
                    ...
                    {
                        string sLocTmp = string.Empty;
                        sSql_Mapping = "select loc, loc1 from loc_mst where LOCID = '" + txtLocID2.Text + "' ";
                        if (clsDB.FunRsSql(sSql_Mapping, ref dbRS))
                        {
                            while (dbRS.Read())
                            {
                ...
                {
                    string sLocTmp = string.Empty;
                    sSql_Mapping = "select M.loc, M.loc1 from loc_mst as M left join Loc_Dtl as D ON M.LOC=D.LOC where D.ITEMNO = '" + txtItemNo1.Text + "' ";
                    if (clsDB.FunRsSql(sSql_Mapping, ref dbRS))
                    {
                        while (dbRS.Read())
                        {
                ...
                {
                    string sLocTmp = string.Empty;
                    sSql_Mapping = "select M.loc, M.loc1 from loc_mst as M left join Loc_Dtl as D ON M.LOC=D.LOC where D.CUSTOMER = '" + txtCustomer.Text + "' ";
                    if (clsDB.FunRsSql(sSql_Mapping, ref dbRS))
                    {
                        while (dbRS.Read())
                        {
            ...
            if (clsDB.FunRsSql(sSql, ref dbRS))
            {
                while (dbRS.Read())
                {
                    ...
                    tmpGrid1[iCol_SubLoc, tmpGrid1.RowCount - 1].Value = dbRS["LOC1"].ToString();
            ...
            for (int i = 0; i <= tmpGrid1.RowCount - 1; i++)
            {
        ...
        private void Grid1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if ((Grid1[iCol_LocSts, e.RowIndex].Value.ToString() == "S") || (Grid1[iCol_LocSts, e.RowIndex].Value.ToString() == "E"))
            {
                //必須是庫存儲位和或空料盒儲位
            }
            else
            ...
            for (i = 0; i <= aLoc.Length - 1; i++)
            {
                if (aLoc[i] != "")
                {
                    ...
                    if (bFlagTmp == true)
                    {
                        ...
                        //判斷左右
                        int iCrn_Row = clsTool.INT(sCrane_Row);

                        //if ((sCrane_Row == "2") || (sCrane_Row == "3"))
                        if ((iCrn_Row == 2) || (iCrn_Row == 3))
                        {
                            sSNO1 = "1";
                            sSNO2 = "2";
                        }
                    ...
                    {
                        sStnNo = clsASRS.FunGetStnNoByLoc_SPIL_ZX(aLoc[i]);
                    }
                    ...
                    if (bCmdFlag == true)
                    {
                        ...
                        {
                            sSQL = "UPDATE LOC_MST SET LOCSTS = 'C' WHERE LOC = '" + aLoc[i] + "' AND LOCSTS = 'E' ";
                        }
                    ...
                    if (bCmdFlag == true)
                    {
                        if (sLoc1 != "")
                        {
                            if (sLocSts_B == "S")
                            {
                                sSQL = "UPDATE LOC_MST SET LOCSTS = 'C' WHERE LOC = '" + sLoc1 + "' AND LOCSTS = 'S' ";
                            }
                            else
                            {
                                sSQL = "UPDATE LOC_MST SET LOCSTS = 'C' WHERE LOC = '" + sLoc1 + "' AND LOCSTS = 'E' ";
                            }
                            if (clsDB.FunExecSql(sSQL) == false)
                            {
                                bCmdFlag = false;
                    ...
                    if (bCmdFlag == true)
                    {
                        ...
                        aCmdMst.IOTYP = clsASRS.gsIOTYPE_Cycle;
                        if (sLocSts_A == "S")
                        {
                        ...
                        aCmdMst.NEWLOC = "";
                        aCmdMst.PROGID = clsASRS.gsIOTYPE_Cycle_PID;
        ...
        private void btnHelp_Cust_Click(object sender, EventArgs e)
        {
            clsASRS.gsHelpStyle = "CUSTOMER";
            Array.Resize<string>(ref clsASRS.gsHelpRtnData, 1);

            frmHelp frmHelp = new frmHelp();
            frmHelp.ShowDialog();

            txtCustomer.Text = clsASRS.gsHelpRtnData[0];
        }

        private void txtLocMapping_TextChanged(object sender, EventArgs e)
        {
            
        }

        private void txtLocIDMapping_TextChanged(object sender, EventArgs e)
        {
