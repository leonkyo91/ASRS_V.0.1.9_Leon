...
namespace ASRS
{
    public partial class frm_ITEM_CYCLE : Form
    {
        ...
        public frm_ITEM_CYCLE()
        {
            InitializeComponent();
        }

        private void frm_ITEM_CYCLE_Load(object sender, EventArgs e)
        {
            GridInit();
            FormInit();
        }
        ...
        private void GridSetRange(ref DataGridView oGrid)
        {
            ...
            oGrid.Columns[iCol_TRANSACTION_TYPE_ID].Width = 100; oGrid.Columns[iCol_TRANSACTION_TYPE_ID].Name = "Transaction Type"; oGrid.Columns[iCol_Store].Visible = false;
        ...
        private void FormInit()
        {
            ...
            dtpDate1.Value = System.DateTime.Now;
            dtpDateF.Value = System.DateTime.Now;
            dtpDateT.Value = System.DateTime.Now;            
        }
        ...
        private void SubClear()
        {
            ...
            dtpDateF.Value = System.DateTime.Now;
            dtpDateT.Value = System.DateTime.Now;
            chkNoCycle.Checked = false;
            //cboStnNo.SelectedIndex = 0;
            txtLocCnt.Text = "";
            Grid1.RowCount = 0;
            tmpGrid1.RowCount = 0;
            tmpGrid2.RowCount = 0;
        ...
        private void SubQuery()
        {
            ...
            txtItemNoF.Text = txtItemNoF.Text.Trim();
            txtItemNoT.Text = txtItemNoT.Text.Trim();
            txtLotNoF.Text = txtLotNoF.Text.Trim();
            txtLotNoT.Text = txtLotNoT.Text.Trim();
            txtDeviceF.Text = txtDeviceF.Text.Trim();
            txtDeviceT.Text = txtDeviceT.Text.Trim();

            #region SQL
            sSql = "SELECT M.LOCSTS,M.LOCID,M.AVAIL,M.MIXQTY,D.* FROM LOC_MST M,LOC_DTL D WHERE M.LOC=D.LOC AND M.LOC IN (";
            sSql = sSql + "(SELECT DISTINCT M.LOC FROM LOC_MST M,LOC_DTL D WHERE M.LOC=D.LOC ";

            if (txtItemNoF.Text != "")
            {
                if (txtItemNoT.Text != "")
                {
                    sSql = sSql + "AND D.ITEMNO >= '" + txtItemNoF.Text + "' AND D.ITEMNO <= '" + txtItemNoT.Text + "' ";
                }
                else
                {
                    if (txtItemNoF.Text.IndexOf("%", 0) > 0)
                    {
                        sSql = sSql + "AND D.ITEMNO LIKE '" + txtItemNoF.Text + "' ";
                    }
                    else
                    {
                        sSql = sSql + "AND D.ITEMNO = '" + txtItemNoF.Text + "' ";
                    }
                }
            }
            ...
            if (txtLotNoF.Text != "")
            {
                if (txtLotNoT.Text != "")
                {
                    sSql = sSql + "AND D.LOTNO >= '" + txtLotNoF.Text + "' AND D.LOTNO <= '" + txtLotNoT.Text + "' ";
                }
                else
                {
                    if (txtLotNoF.Text.IndexOf("%", 0) > 0)
                    {
                        sSql = sSql + "AND D.LOTNO LIKE '" + txtLotNoF.Text + "' ";
                    }
                    else
                    {
                        sSql = sSql + "AND D.LOTNO = '" + txtLotNoF.Text + "' ";
                    }
                }
            }
            ...
            if (txtDeviceF.Text != "")
            {
                if (txtDeviceT.Text != "")
                {
                    sSql = sSql + "AND D.DEVICE >= '" + txtDeviceF.Text + "' AND D.DEVICE <= '" + txtDeviceT.Text + "' ";
                }
                else
                {
                    if (txtDeviceF.Text.IndexOf("%", 0) > 0)
                    {
                        sSql = sSql + "AND D.DEVICE LIKE '" + txtDeviceF.Text + "' ";
                    }
                    else
                    {
                        sSql = sSql + "AND D.DEVICE = '" + txtDeviceF.Text + "' ";
                    }
                }
            }

            if (chkTREANSACTION_DATE1.Checked == true)
            {
                    sSql = sSql + "AND D.TRANSACTION_DATE ";
                    if (optBefore.Checked == true)
                    { 
                        sSql = sSql + " <= ";
                    }
                    else
                    ...
                    sSql = sSql + "'" + dtpDate1.Value.ToString("yyyy/MM/dd") + "' ";
            }
            if (chkTREANSACTION_DATE2.Checked == true)
            {
                    sSql = sSql + " AND D.TRANSACTION_DATE >= '" + dtpDateF.Value.ToString("yyyy/MM/dd") + "' ";
                    sSql = sSql + " AND D.TRANSACTION_DATE <= '" + dtpDateT.Value.ToString("yyyy/MM/dd") + "' ";
            }
            if (chkNoCycle.Checked == true)
            {
                sSql = sSql + " AND (M.CYCLEDATE<>' ' OR M.CYCLEDATE<>'') ";
            }
        ...
        private void chkTREANSACTION_DATE1_CheckedChanged(object sender, EventArgs e)
        {
            if (chkTREANSACTION_DATE1.Checked == true)
            {
                chkTREANSACTION_DATE2.Checked = false;
        ...
        private void chkTREANSACTION_DATE2_CheckedChanged(object sender, EventArgs e)
        {
            if (chkTREANSACTION_DATE2.Checked == true)
            {
                chkTREANSACTION_DATE1.Checked = false;
        ...
        private void Grid1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (Grid1[iCol_LocSts, e.RowIndex].Value.ToString() == "E")
            {
                clsMSG.ShowWarningMsg("空料盤不可以做盤點!!");
                return;
            }
            ...
            else
            {
                for (i = 0; i <= Grid1.RowCount - 1; i++)
                {
                    if (Grid1[iCol_Loc, i].Value.ToString() == sLoc)
                    {
                        clsASRS.SetGridSeletRowColorHead(ref Grid1, i, 1); //Sel
                    }
                }
            }
            ...
            for (i = 0; i <= aLoc.Length - 1; i++)
            {
                ...
                if (aLoc[i] != "")
                {
                    ...
                    if (bCmdFlag == true)
                    {
                        ...
                        aCmdMst.IOTYP = clsASRS.gsIOTYPE_Cycle_Item;
                        aCmdMst.AVAIL = "100";  // 彰化廠
                        aCmdMst.MIXQTY = "1";   // 彰化廠
                        aCmdMst.NEWLOC = "";
                        aCmdMst.PROGID = clsASRS.gsIOTYPE_Cycle_Item_PID;
