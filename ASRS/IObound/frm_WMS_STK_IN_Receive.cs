...
namespace ASRS
{
    public partial class frm_WMS_STK_IN_Receive : Form
    {
        ...
        public frm_WMS_STK_IN_Receive()
        {
            InitializeComponent();
        }

        private void frm_WMS_STK_IN_Receive_Load(object sender, EventArgs e)
        {
            GridInit();
            FormInit();
        }
        ...
        private void SubReadFOSBID(string sValue)
        {
            ...
            if (SubReadFosbID_get_LocID() == false) { btnCls.PerformClick(); return; }

            if (clsDB.FunOpenDB() == false) { clsMSG.ShowErrMsg(clsMSG.MSG.OPEN_DB_NG); return; }

            //2.判別 FOSB條碼 是否重覆 & 判別 FOSB條碼 是否為CMD_DTL的資料
            if (SubReadFosbID_OverLay(sValue) == false) { btnCls.PerformClick(); clsDB.FunClsDB(); return; }

            //3.判別 FOSB條碼是否在儲位中或命令中
            if (SubReadFOSBID_InLoc(sValue) == false) { btnCls.PerformClick(); clsDB.FunClsDB(); return; }
        ...
        private void SubReturnTray()
        {
            ...
            {                
                ...
                if (FunDelWithCmd_CMD_MST_empty() == false)
                {
                    clsDB.FunCommitCtrl("ROLLBACK");
                    clsDB.FunClsDB();
                    txtScan.Focus();
        ...
        private bool FunDelWithCmd_CMD_MST_empty()
        {
            string sSQL = "";
            sSQL = "UPDATE CMD_MST SET SCAN = 'Y'";
            sSQL = sSQL + ", MIXQTY = '0', AVAIL = 0 ";   //Importmant
            sSQL = sSQL + "WHERE CMDSNO = '" + get_CmdSno + "' AND LOCID = '" + get_LocID + "' AND CMDSTS <= '6' ";
            if (clsDB.FunExecSql(sSQL) == true)
            {
                return true;
